...
class MapFunctions {
    ...
    //游戏存档
    static void mapSave(String[][][] map, int floor, int[] position,
        ...
        for (int i = 0; i <= 4; i++) {
            // 创建一个 File 实例
            File mapSave = new File("map/save" + i + ".txt");
            // 创建一个文件
    ...
    //游戏读档
    static String[][][] mapLoad() throws FileNotFoundException {
        String[][][] loadedInformation = new String[6][13][13];

        //按行将地图信息写入数组前5组
        String[][] MapRead = new String[6][13];
        for (int i = 0; i <= 4; i++) {
            Scanner scanner = new Scanner(new File("map/save" + i + ".txt"));
            for (int j = 0; j <= 12; j++) {
                MapRead[i][j] = scanner.nextLine() + "\n";
            }
            for (int j = 0; j <= 9; j++) {
    ...
    //勇者上楼时，位置变为新一层向下楼梯口的位置
    static int[] changeFloorPosition(boolean upOrDown, int floor) {
        int[] FloorPosition = new int[2];
        if (upOrDown) {
            switch (floor) {
                case 1:
                    FloorPosition[0] = 1; FloorPosition[1] = 6; break;
                case 2:
                    FloorPosition[0] = 7; FloorPosition[1] = 6; break;
                case 3:
                    FloorPosition[0] = 6; FloorPosition[1] = 11; break;
                case 4:
                    FloorPosition[0] = 11; FloorPosition[1] = 6; break;
            }
        } else {
            switch (floor) {
                case 0:
                    FloorPosition[0] = 2; FloorPosition[1] = 6; break;
                case 1:
                    FloorPosition[0] = 7; FloorPosition[1] = 6; break;
                case 2:
                    FloorPosition[0] = 6; FloorPosition[1] = 11; break;
